...
import (
	...
	"go.senan.xyz/mrtag/cmd/internal/mrtagflag"
...
func main() {
	...
	mrtagflag.DefaultClient()
	var (
		cfg = mrtagflag.Config()
	)
	mrtagflag.Parse()
	...
	case "move", "copy":
		...
		var importCondition mrtag.ImportCondition
		if *yes {
			importCondition = mrtag.Confirm
		}
...
func runOperation(
	ctx context.Context, cfg *mrtag.Config,
	op mrtag.FileSystemOperation, srcDir string, cond mrtag.ImportCondition, useMBID string,
) error {
	r, err := mrtag.ProcessDir(ctx, cfg, op, srcDir, cond, useMBID)
	if err != nil && !errors.Is(err, mrtag.ErrScoreTooLow) && !errors.Is(err, mrtag.ErrTrackCountMismatch) {
...
func runSync(ctx context.Context, cfg *mrtag.Config, dirs []string, ageYounger, ageOlder time.Duration, dryRun bool, numWorkers int) error {
	start := time.Now()
	...
	for range numWorkers {
		...
		go func() {
			...
			ctxConsume(ctx, leaves, func(dir string) {
				st.saw.Add(1)
				r, err := syncDir(ctx, cfg, ageYounger, ageOlder, mrtag.Move{DryRun: dryRun}, dir)
...
func syncDir(ctx context.Context, cfg *mrtag.Config, ageYounger, ageOlder time.Duration, op mrtag.FileSystemOperation, srcDir string) (*mrtag.SearchResult, error) {
	...
	r, err := mrtag.ProcessDir(ctx, cfg, op, srcDir, mrtag.HighScoreOrMBID, "")
	if err != nil {
		return nil, err
	}
...
func parseOperation(name string, dryRun bool) mrtag.FileSystemOperation {
	switch name {
	case "copy":
		return mrtag.Copy{DryRun: dryRun}
	case "move":
		return mrtag.Move{DryRun: dryRun}
	}
	return nil
}
