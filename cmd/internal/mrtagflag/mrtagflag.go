...
import (
	...
	"go.senan.xyz/mrtag"
	"go.senan.xyz/mrtag/addon"
	"go.senan.xyz/mrtag/clientutil"
	"go.senan.xyz/mrtag/notifications"
...
func DefaultClient() {
	chain := clientutil.Chain(
		...
		clientutil.WithUserAgent(fmt.Sprintf(`%s/%s`, mrtag.Name, mrtag.Version)),
	)
...
func Parse() {
	...
	defaultConfigPath := filepath.Join(userConfig, mrtag.Name, "config")
	...
	flag.Parse()
	flagconf.ReadEnvPrefix = func(_ *flag.FlagSet) string { return mrtag.Name }
	flagconf.ParseEnv()
	flagconf.ParseConfig(*configPath)

	if *printVersion {
		fmt.Printf("%s %s\n", flag.CommandLine.Name(), mrtag.Version)
		os.Exit(0)
	}
	if *printConfig {
...
func Config() *mrtag.Config {
	var cfg mrtag.Config
